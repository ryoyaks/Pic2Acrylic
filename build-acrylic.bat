...
REM Pic2Acrylic one-click pipeline: uv-managed stage 1 + Blender stage 2.
REM Usage: drag a parts folder onto this file, or: build-acrylic.bat <parts_dir>
...
where uv >nul 2>nul
if errorlevel 1 (
  echo ERROR: uv is required for stage 1. Install it from https://docs.astral.sh/uv/
  pause
  exit /b 1
)

echo.
echo === Stage 1: prep_masks (uv) ===
uv run --project "%HERE%" python "%HERE%prep_masks.py" "%SRC%" -o "%PREP%"
if errorlevel 1 (
